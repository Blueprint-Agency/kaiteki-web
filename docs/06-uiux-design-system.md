...
- **Evidence over aspiration.** With testimonials prohibited (`docs/02 §8.1`), the imagery system (§2.5) is built on real clinics, real devices, real doctors, and calm material textures — original photography, no stock (`docs/03 §5` Kaiteki target row). Concern pages additionally carry captioned before/after results under [ADR-0001](adr/0001-before-after-imagery.md); the ADR's conditions (caption, disclaimer, native-width cap) are design requirements, not copy notes.
