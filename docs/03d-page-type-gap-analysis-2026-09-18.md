...
3. **Our moat is built but switched off.** Every rival lacks per-page medical schema and a named MMC reviewer. We have both — yet all 14 concern pages ship with no reviewer (the sign-off ledger is empty), and 36 technology pages emit placeholder review dates.
...
| Named reviewer + MMC + linked doctor page | ⚠️ named, LCP, no MMC, no link | ❌ admin author | ❌ | ✅ treatments · **❌ concerns (ledger empty)** · ⚠️ technology (placeholder dates) | **The ownable gap.** Nobody else does it; we do it on 19 of 69 pages |
