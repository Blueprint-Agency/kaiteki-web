...
1. **The law is the gate, not a reviewer note.** Every page classifies as either **advertisement** (treatments, locations, promotions, home) or **educational** (blog, concern guides in informational register) before a word is written (`docs/02 §8.3`). Advertisement pages carry KKLIU + promoter details and pass MAB screening; educational pages carry references, disclaimer, and **no price/offer/booking pressure**. No testimonials, no superlatives, no outcome guarantees — anywhere (`docs/02 §8.1`). **Before/after imagery is the one exception**, reinstated for concern pages by [ADR-0001](adr/0001-before-after-imagery.md) under the conditions recorded there.
