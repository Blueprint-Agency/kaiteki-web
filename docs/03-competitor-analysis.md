...
>
> **⚠️ Benchmark set superseded (2026-09-06).** A live-SERP re-run — [`03a-competitor-gap-analysis-2026-09.md`](./03a-competitor-gap-analysis-2026-09.md) — found that **Nexus Clinic and Beverly Wilshire do not rank** for the head queries, and Premier holds only a local-pack slot. They appear on these SERPs inside third-party listicles, not on their own pages. The real structural rival is **Dr Chong Clinic** (24 branches, per-branch `/location/` pages, bilingual `/ms/`), which this doc never analysed. Two claims below are retired by 03a: **"no competitor has location pages"** and **"JB/Kota Kinabalu are uncontested"**. Cleo remains the correct primary benchmark, and the medical-schema / named-reviewer wedge survived re-testing.
>
