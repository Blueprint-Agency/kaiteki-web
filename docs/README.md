...
| 15 | [`15-four-week-plan-2026-09-21.md`](./15-four-week-plan-2026-09-21.md) | **Four-week execution plan (21 Sep – 18 Oct 2026)** for `03d`: day-1 client asks, then week by week — sign-offs and blog links, technology template upgrade with per-branch availability, acne-scars and melasma pages plus the remaining treatment block sets, FAQ depth and a measured comparison against the week-1 baseline. Owner, dependency and done-criteria per item. |
| 15a | [`15a/README.md`](./15a/README.md) | **Day-1 client pack** for `15`: the message to the client, Sheet A (medical sign-off for all 69 pages, CSV), Sheet B (branch × device matrix, CSV), the price options, the AI-crawler decision, and the indexing queue in three dated batches. Records the day-1 corrections: Cloudflare block already clear; treatment reviewers are placeholders too. |
